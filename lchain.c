...
static int64_t mg_chain_bk_end(int32_t max_drop, const mm128_t *z,
  ...
  do {
    ...
    t[i] = 2;
    end_i = i = p[i];
    // z[k].x is score of current anchor,
    // f[i] is score of predecessor,
    // Make sure drop is not too big.
...
static inline int32_t comput_sc(const mm128_t *ai, const mm128_t *aj,
  ...
  if (dd || dg > q_span) {
    ...
    log_pen =
        dd >= 1 ? mg_log2(dd + 1) : 0.0f; // mg_log2() only works for dd>=2
    if (is_cdna || sidi != sidj) {
      if (sidi != sidj && dr == 0)
        ...
        sc -= (int)(lin_pen < log_pen
                        ? lin_pen
                        : log_pen); // deletion or jump between paired ends
      else
...
mm128_t *mg_lchain_dp(
    ...
    int n_seg, int64_t n, mm128_t *a, int *n_u_, uint64_t **_u,
  ...
  for (i = 0, max_ii = -1; i < n; ++i) {
    ...
    while (st < i &&
           (a[i].x >> 32 != a[st].x >> 32 || a[i].x > a[st].x + max_dist_x))
      ++st;
    if (i - st > max_iter)
      st = i - max_iter;
    for (j = i - 1; j >= st; --j) {
    ...
    if (max_ii >= 0 && max_ii < end_j) {
      int32_t tmp;
      tmp = comput_sc(&a[i], &a[max_ii], max_dist_x, max_dist_y, bw,
                      chn_pen_gap, chn_pen_skip, is_cdna, n_seg);
      if (tmp != INT32_MIN && max_f < tmp + f[max_ii])
        max_f = tmp + f[max_ii], max_j = max_ii;
    }
    f[i] = max_f, p[i] = max_j;
    v[i] = max_j >= 0 && v[max_j] > max_f
               ? v[max_j]
               : max_f; // v[] keeps the peak score up to i; f[] is the score
                        // ending at i, not always the peak
    if (max_ii < 0 ||
        (a[i].x - a[max_ii].x <= (int64_t)max_dist_x && f[max_ii] < f[i]))
      max_ii = i;
    if (mmax_f < max_f)
      mmax_f = max_f;
  }

  u = mg_chain_backtrack(km, n, f, p, v, t, min_cnt, min_sc, max_drop, &n_u,
                         &n_v);
...
static inline int32_t comput_sc_simple(const mm128_t *ai, const mm128_t *aj,
...
mm128_t *mg_lchain_rmq(int max_dist, int max_dist_inner, int bw,
  ...
  if (max_dist < bw)
    max_dist = bw;
  if (max_dist_inner <= 0 || max_dist_inner >= max_dist)
    max_dist_inner = 0;
  KMALLOC(km, p, n);
  KMALLOC(km, f, n);
  KCALLOC(km, t, n);
  ...
  for (i = i0 = 0; i < n; ++i) {
    ...
    if (i0 < i && a[i0].x != a[i].x) {
      ...
      for (j = i0; j < i; ++j) {
        q = kmp_alloc_rmq(mp);
        q->y = (int32_t)a[j].y, q->i = j,
        q->pri =
            -(f[j] + 0.5 * chn_pen_gap * ((int32_t)a[j].x + (int32_t)a[j].y));
    ...
    while (st < i &&
           (a[i].x >> 32 != a[st].x >> 32 || a[i].x > a[st].x + max_dist ||
            krmq_size(head, root) > cap_rmq_size)) {
    ...
    if (max_dist_inner >
        0) { // similar to the block above, but applied to the inner tree
    ...
    if ((q = krmq_rmq(lc_elem, root, &lo, &hi)) != 0) {
      ...
      sc = f[j] + comput_sc_simple(&a[i], &a[j], chn_pen_gap, chn_pen_skip,
                                   &exact, &width);
      ...
      if (!exact && root_inner && (int32_t)a[i].y > 0) {
        ...
        if (lo) {
          ...
          while ((q = krmq_at(&itr)) != 0) {
            if (q->y < (int32_t)a[i].y - max_dist_inner)
              break;
            ++n_rmq_iter;
            j = q->i;
            sc = f[j] + comput_sc_simple(&a[i], &a[j], chn_pen_gap,
                                         chn_pen_skip, 0, &width);
            if (width <= bw) {
              if (sc > max_f) {
                max_f = sc, max_j = j;
                if (n_skip > 0)
    ...
    // set max
    assert(max_j < 0 ||
           (a[max_j].x < a[i].x && (int32_t)a[max_j].y < (int32_t)a[i].y));
    f[i] = max_f, p[i] = max_j;
    v[i] = max_j >= 0 && v[max_j] > max_f
               ? v[max_j]
               : max_f; // v[] keeps the peak score up to i; f[] is the score
    ...
    if (mmax_f < max_f)
      mmax_f = max_f;
    if (max_rmq_size < krmq_size(head, root))
      max_rmq_size = krmq_size(head, root);
  }
  km_destroy(mem_mp);
