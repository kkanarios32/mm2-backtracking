...
typedef struct {
  ...
  int32_t dp_score, dp_max,
      dp_max2; // DP score; score of the max-scoring segment; score of the best
               // alternate mappings
  uint32_t n_ambi : 30,
...
typedef struct {
  ...
  float max_clip_ratio; // drop an alignment if BOTH ends are clipped above this
                        // ratio
  ...
  int64_t
      mini_batch_size; // size of a batch of query bases to process in parallel
  int64_t max_sw_mat;
  int64_t cap_kalloc;
...
typedef enum { MM_TIME_SEED = 0, MM_TIME_CHAIN, MM_TIME_ALIGN } mm_thr_timer_t;
...
void mm_map_bt(const mm_idx_t *mi, uint64_t **u, int l_seq, const char *seq,
               int *n_regs, mm_tbuf_t *b, const mm_mapopt_t *opt,
               const char *name);

void mm_map_frag_bt(const mm_idx_t *mi, int n_segs, const int *qlens,
                    const char **seqs, int *n_regs, uint64_t **u, mm_tbuf_t *b,
                    const mm_mapopt_t *opt, const char *qname);
